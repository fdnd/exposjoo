<script>
  ...
  onMount(() => {
    ...
    let gentleEase = CustomEase.create('custom', 'M0,0 C0.397,0 0.44,1.219 1,1')
    ...
    tl.to(`h1:not(${sizes.largerItems[0]}, ${sizes.largerItems[1]}) .intro__title`, {
      y: orientation === 'landscape' ? '70%' : '50%',
      duration: 1,
      ease: 'power2.inOut',
    }, '<')
    tl.to(`h1${sizes.largerItems[0]}, h1${sizes.largerItems[1]}`, {
    ...
    tl.to(`h1${sizes.largerItems[0]} .intro__title`, {
      y: '40%',
      duration: 1,
      ease: 'power2.inOut',
    }, '<')
    tl.to(`h1${sizes.largerItems[1]} .intro__title`, {
    ...
    tl.to('h1:nth-child(even) .intro__title', {
      x: '-115%',
      duration: 1,
      ease: 'power2.inOut',
      onComplete: () => {
...
{#if isVisible}
  <div class="intro" inert="" style="--title-count: {titleCount}" bind:this={intro}>
    {#each Array(titleCount) as _, i (i)}
      <h1 class="xlarge-title" style="--index: {titleCount - 1 - i};">
